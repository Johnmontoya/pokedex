...
export class PokemonService {
  ...
  async findOne(id: string) {
    ...
    if(!pokemon) throw new NotFoundException(`El pokemon con id o nombre no existe`)
    return pokemon;
  }
  ...
  async remove(id: string) {
    ...
    if( deletedCount === 0){
      throw new BadRequestException(`El pokemon con id ${ id } no encontrado`)
    }
    return;
  }
  ...
  private handleExceptions( error: any){
    ...
    console.log(error)
    throw new InternalServerErrorException('No se puede crear el pokemon, problemas de servidor')
  }
}
